            ...
            for subsubfolder_name in os.listdir(original_subfolder_path):
                ...
                if os.path.isdir(original_subsubfolder_path):
                    ...
                    for img_path in tqdm(original_file_list, dynamic_ncols=True, disable=len(original_file_list) <= 1):
                        ...
                        df1 = pd.concat(
                            [df1, pd.DataFrame([{'original_path': img_path, 'original_pro': prob, 'flag': flag}])],
                            ignore_index=True)
                    ...
                    df = pd.concat([df, pd.DataFrame(
                        [{'name': subsubfolder_name, 'pro': predict, 'flag': flag, 'optical_pro': optical_predict,
                          'original_pro': original_predict}])], ignore_index=True)
        else:
